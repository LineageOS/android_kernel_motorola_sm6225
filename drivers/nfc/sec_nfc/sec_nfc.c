...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
	...
	ret = request_threaded_irq(client->irq, NULL, sec_nfc_irq_thread_fn,
		IRQF_TRIGGER_RISING | IRQF_ONESHOT, SEC_NFC_DRIVER_NAME, info);
	if (ret < 0) {
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	if (dev->of_node && !mmi_device_is_available(dev->of_node)) {
		dev_err(dev,"%s : mmi: device not supported\n", __func__);
		return -ENODEV;
	}
	else {
		dev_err(dev,"%s : supported device found\n", __func__);
	}
