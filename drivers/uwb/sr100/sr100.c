...
static int sr100_hw_setup(struct sr100_spi_platform_data* platform_data,
  ...
  ret = gpio_direction_output(sr100_dev->vdd_1v8_gpio, 0);
  if (ret < 0) {
  ...
  ret = gpio_direction_output(sr100_dev->vdd_1v8_rf_gpio, 0);
  if (ret < 0) {
  ...
  ret = gpio_direction_output(sr100_dev->vbat_3v6_gpio, 0);
  if (ret < 0) {
...
#endif
  pr_info("sr100 : irq_gpio = %d, ce_gpio = %d, spi_handshake_gpio = %d, rtc_sync_gpio =  %d \n",
          pdata->irq_gpio, pdata->ce_gpio,pdata->spi_handshake_gpio,pdata->rtc_sync_gpio);
  return 0;
}
/******************************************************************************
...
static int sr100_probe(struct spi_device* spi) {
  ...
  if (spi->dev.of_node && !mmi_device_is_available(spi->dev.of_node)) {
    SR100_ERR_MSG("%s : mmi: device not supported\n", __func__);
    return -ENODEV;
  } else {
    SR100_ERR_MSG("%s : supported device found\n", __func__);
  }
