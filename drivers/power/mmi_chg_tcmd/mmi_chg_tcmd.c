 ...
 *
 * Adding this driver is for mmi chg tcmd's kernel interface.
 * So after then, tcmd app do not  need to change file access folder's name.
 * It will more convinient for all projects with different charger IC.
 *
...
#include <linux/power/mmi_chg_tcmd.h>
#ifdef USE_LIST_HEAD
#include <linux/list.h>
#endif

#define DRIVERNAME	"mmi_chg_tcmd"

/*-20 ~ 80 num is 2*10 + 1*/
#define NTC_TABLE_VALID_NUM	21


struct mmi_chg_tcmd_data {
...
#else
static struct mmi_chg_tcmd_client* chg_client;
static struct mmi_chg_tcmd_client* bat_client;
static struct mmi_chg_tcmd_client* pm_adc_client;
static struct mmi_chg_tcmd_client* ap_adc_client;
#endif
...
static int mmi_chg_tcmd_res_to_temp(struct mmi_chg_tcmd_data *data,
					int vol, int vol_ntc, int r_pull);

static int mmi_chg_get_adc_value(struct  mmi_chg_tcmd_data *data,
				struct mmi_chg_tcmd_client* client, int channel, int* val)
{
	int ret;
...
static ssize_t mmi_chg_tcmd_addr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mmi_chg_tcmd_data *data = platform_get_drvdata(pdev);

	return snprintf(buf, PAGE_SIZE, "%x\n", data->reg_addr);
}

static ssize_t mmi_chg_tcmd_addr_store(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(address, S_IWUSR | S_IRUGO, mmi_chg_tcmd_addr_show, mmi_chg_tcmd_addr_store);

static ssize_t mmi_chg_tcmd_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mmi_chg_tcmd_data *data = platform_get_drvdata(pdev);
	unsigned char val;
...
static ssize_t mmi_chg_tcmd_data_store(struct device *dev, struct device_attribute *attr,
									const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct  mmi_chg_tcmd_data *data = platform_get_drvdata(pdev);
...
static DEVICE_ATTR(data, S_IWUSR | S_IRUGO, mmi_chg_tcmd_data_show, mmi_chg_tcmd_data_store);
...
static ssize_t adc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mmi_chg_tcmd_data *data = platform_get_drvdata(pdev);
	struct mmi_chg_tcmd_client *client;
	int channel;
	int ret;
	...
	mmi_chg_get_adc_value(data, client, data->cur_adc_channel, &ret);
...
static ssize_t cpu_temp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mmi_chg_get_adc_value(data, client, channel, &ret);
	ret = mmi_chg_tcmd_res_to_temp(data, data->ntc_v_ref, ret, data->ntc_r_pull);
...
static struct attribute *mmi_chg_tcmd_attrs[] = {
...
ATTRIBUTE_GROUPS(mmi_chg_tcmd);

int mmi_chg_tcmd_get_bat_psy_prop(struct mmi_chg_tcmd_data *data,
										int prop,
										int* value)
{
...
{
#ifdef USE_LIST_HEAD
	struct mmi_chg_tcmd_client *existing;
#endif
...
static int mmi_chg_tcmd_get_ntc_table(struct mmi_chg_tcmd_data *data, int id)
{
	switch (id) {
		case 1:
...
static int mmi_chg_tcmd_parse_dt(struct mmi_chg_tcmd_data *data)
{
	...
	mmi_chg_tcmd_get_ntc_table(data, val);
...
static int mmi_chg_tcmd_probe(struct platform_device *pdev)
{
	struct mmi_chg_tcmd_data *data;
	int ret;
	...
	ret = sysfs_create_groups(&pdev->dev.kobj, mmi_chg_tcmd_groups);
	if (ret) {
		dev_err(&pdev->dev, "%s Failed to create sysfs attributes\n", __func__);
		return ret;
	}

	mmi_chg_tcmd_parse_dt(data);
...
static int mmi_chg_tcmd_remove(struct platform_device *pdev)
{
	sysfs_remove_groups(&pdev->dev.kobj, mmi_chg_tcmd_groups);
...
static struct of_device_id  mmi_chg_tcmd_match_table[] = {
	{ .compatible = "mmi,mmi-chg-tcmd"},
	{}
};


static struct platform_driver mmi_chg_tcmd_driver = {
	.probe	= mmi_chg_tcmd_probe,
	.remove = mmi_chg_tcmd_remove,
	.driver = {
		.name = DRIVERNAME,
		.of_match_table =  mmi_chg_tcmd_match_table,
	},
};

int __init mmi_chg_tcmd_init(void)
{
	int ret;

	ret = platform_driver_register(&mmi_chg_tcmd_driver);

	return ret;
}

void mmi_chg_tcmd_exit(void)
{
	platform_driver_unregister(&mmi_chg_tcmd_driver);
}

module_init(mmi_chg_tcmd_init);
module_exit(mmi_chg_tcmd_exit);

MODULE_DESCRIPTION("mmi-chg-tcmd Driver");
