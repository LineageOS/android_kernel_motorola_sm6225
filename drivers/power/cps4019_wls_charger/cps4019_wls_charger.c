...
static int cps_wls_write_reg(int reg, int value, int byte_len)
{
    ...
    {
        tmp = (value >> (i*8))&0xff;
        if(cps_wls_write((reg&0xffff) + i, tmp) == CPS_WLS_FAIL)     goto write_fail;
    }
    return CPS_WLS_SUCCESS;
...
static int cps_wls_write_word(int addr, int value)
{
    ...
    mutex_lock(&chip->i2c_lock);
    ret = regmap_raw_write(chip->regmap, addr, write_date, 4);
    mutex_unlock(&chip->i2c_lock);
...
static int cps_wls_read_word(int addr)
{
    ...
    mutex_lock(&chip->i2c_lock);
    ret = regmap_raw_read(chip->regmap, addr, read_date, 4);
    mutex_unlock(&chip->i2c_lock);
 ...
 ***************************************************************************************/
    ...
    if(CPS_WLS_FAIL == cps_wls_write_word(0xFFFFFF00, big_little_endian_convert(0x0E000000))) goto update_fail; /*enable 32bit i2c*/
    if(CPS_WLS_FAIL == cps_wls_write_word(0x40040070, big_little_endian_convert(0x0000A061))) goto update_fail; /*write password*/
    if(CPS_WLS_FAIL == cps_wls_write_word(0x40040004, big_little_endian_convert(0x00000008))) goto update_fail; /*reset and halt mcu*/

    if(CPS_WLS_FAIL == cps_wls_program_sram(0x20000000, bootloader_buf, bootloader_length))  goto update_fail;//program sram

    if(CPS_WLS_FAIL == cps_wls_write_word(0x40040010, big_little_endian_convert(0x00000001))) goto update_fail; /*triming load function is disabled*/
    if(CPS_WLS_FAIL == cps_wls_write_word(0x40040004, big_little_endian_convert(0x00000066))) goto update_fail; /*enable remap function and reset the mcu*/
 ...
 ***************************************************************************************/
    ...
    if(CPS_WLS_FAIL == cps_wls_write_word(0x40040004, big_little_endian_convert(0x00000001))) goto update_fail; /*reset all system*/
    msleep(100);
...
static int cps_wls_set_rx_ocp_threshold(int value)
{
    int value_temp;
    cps_reg_s *cps_reg;
    if(value < 100 || value > 1100) return CPS_WLS_FAIL;
...
static ssize_t store_vout_target(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int tmp;
    tmp = simple_strtoul(buf, NULL, 0);
...
static ssize_t store_ocp_thres(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int tmp;
    tmp = simple_strtoul(buf, NULL, 0);
