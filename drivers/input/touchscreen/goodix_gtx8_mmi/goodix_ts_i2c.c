...
int goodix_set_i2c_doze_mode(struct goodix_ts_device *dev, int enable)
{
	...
	if (enable) {
		...
		if (dev->doze_mode_set_count == 0) {
			w_data = 0xCC;
			for (i = 0; i < 3; i++) {
				result = goodix_i2c_write_trans(dev, 0x30F0,
					&w_data, 1);
				if (!result) {
					result = 0;
					goto exit;
				}
				usleep_range(GTP_1000_DLY_US, GTP_1100_DLY_US);
			}
			if (i >= 3)
		...
		if (dev->doze_mode_set_count == 1) {
			...
			for (i = 0; i < 3; i++) {
				for (j = 0; j < 3; j++) {
					goodix_i2c_read_trans(dev, 0x3100,
						&r_data, 1);
					if (r_data == 0xBB) {
						result = 0;
						goto exit;
					} else if (r_data == 0xAA) {
						usleep_range(GTP_10000_DLY_US,
							GTP_10100_DLY_US);
						continue;
					} else {
						w_data = 0xAA;
...
#define IS_CHAR(c)	(((c) >= 'A' && (c) <= 'Z')\
	...
	if (!pid_read_len || pid_read_len > 8 ||
			!vid_read_len || vid_read_len > 8) {
...
static int goodix_close_hidi2c_mode(struct goodix_ts_device *ts_dev)
{
	...
	for (try_times = 0; try_times < 3; try_times++) {
		if (ts_cmd.initialized) {
			...
			usleep_range(GTP_100000_DLY_US, GTP_110000_DLY_US);
					      ...
					      u8 *config_data, u32 config_len)
{
	...
	for (i = 0; i < 20; i++) {
		...
		if (r < 0 || cmd_flag == 0xFF)
			break;
		usleep_range(GTP_5000_DLY_US, GTP_5200_DLY_US);
	}
	if (cmd_flag != 0xFF) {
		...
		struct goodix_request_data *request_data)
{
	...
	r = goodix_i2c_read_trans(dev, dev->reg.fw_request,
		buffer, 1);/*TS_REG_REQUEST*/
	if (r < 0)
		return r;
...
clear_requ:
	buffer[0] = 0x00;
	r = goodix_i2c_write_trans(dev, dev->reg.fw_request,
		buffer, 1);/*TS_REG_REQUEST*/
	return r;
}
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	if (!buffer) {
		buffer = kzalloc(4 + BYTES_PER_COORD * max_touch_num, GFP_KERNEL);
		if (!buffer)
			return -ENOMEM;
	}
	...
	/*"0 ~ touch_num - 2" is finger,
	 * "touch_num - 1" may be a finger or a pen
	 */
...
exit_clean_sta:
	/* handshake */
	buffer[0] = 0x00;
	goodix_i2c_write_trans(dev, dev->reg.coor,
		buffer, 1);/*TS_REG_COORDS_BASE*/
	return r;
}
