...
#if (defined(CONFIG_FB) || defined(CONFIG_DRM)) && !defined(CONFIG_MMI_PANEL_NOTIFICATIONS)
			...
			void *unused)
{
	...
	if (!IS_ERR(rmi4_data->vdd_quirk)) {
		regulator_force_disable(rmi4_data->vdd_quirk);
...
#if defined(CONFIG_DRM) && !defined(CONFIG_MMI_PANEL_NOTIFICATIONS)
static int synaptics_dsx_panel_cb(struct notifier_block *nb,
		unsigned long event, void *data)
{
...
#elif defined(CONFIG_FB) && !defined(CONFIG_MMI_PANEL_NOTIFICATIONS)
static int synaptics_dsx_panel_cb(struct notifier_block *nb,
		unsigned long event, void *data)
{
