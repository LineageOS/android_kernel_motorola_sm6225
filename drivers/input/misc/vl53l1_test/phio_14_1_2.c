...
static void RangingDataProcess(int dev,
		struct RangingDataProcess_t *pData,
		VL53L1_MultiRangingData_t *pRange,
...
static void help(int do_exit)
{
	fprintf(stderr,	"Usage: phio <option1> <option2> ....\n"
			...
			"\t-Q \"no_of_range distance_mode_enable HALI HALH MALI MALH timing_budget_enable SignalLow SignalHigh\"  poll histogram ranging with distance mode and/or timing budget auto adjustment \n"
 ...
 */
char string_options[]="L:l:d:?m::M::o:O::p::P:Q:r::sSt::z::Z:RX:F:c:C:D::E::U::f::a::yYT::IA::B::w:W:N:g:G:h::j::J:H:";
...
void print_target1xdata_avg(FILE * fi, VL53L1_TargetRangeData_t *range_data,
...
void print_mz_data_avg(FILE *fi, VL53L1_MultiRangingData_t *data,
...
int run_zone_loop(int n_loops)
{
	...
	for(i=0; i< n_loops; i++) {
		...
		printf("#%c%5d dts=%8d, crosstalk_offset_kcps = %d\t", is_sysfs ? 'S' : 'I',i, i==0 ? 0 : cur_ts - prev_ts, cal.data.customer.algo__crosstalk_compensation_plane_offset_kcps);
		dev_print_mz_data();
		prev_ts = cur_ts;
	}
	if (is_sysfs)
		...
		struct RangingDataProcess_t *RangingData)
{
	...
	for(i=0; i< n_loops; i++) {
		...
		stmvl53l1_get_distance_mode(dev_fd, &PreviousDM);
		stmvl53l1_get_timing_budget(dev_fd, &PreviousTM);
		RangingDataProcess(dev_fd,
				RangingData,
				&dev_mz_range_data,
				&AvgRangeMilliMeter,
				&ConfLevel);
		stmvl53l1_get_distance_mode(dev_fd, &CurrentDM);
		stmvl53l1_get_timing_budget(dev_fd, &CurrentTM);
...
int perform_offset_calibration(int mode, int distance, float reflectance)
{
	 ...
	 * mode = 5 ==> VL53L1_CALIBRATION_OFFSET_PER_VCSEL
	 * 			internal mode=VL53L1_CALIBRATION_OFFSET=2 it doesn't matter
	 * mode = 6 ==> VL53L1_CALIBRATION_OFFSET_ZERO_DISTANCE
	 * 			internal mode=VL53L1_CALIBRATION_OFFSET=2 it doesn't matter
	 * */
	...
	break;
	case 5:
		calibration_type = VL53L1_CALIBRATION_OFFSET_PER_VCSEL;
		param1 = distance;
		param2 = 0;
	break;
	case 6:
		calibration_type = VL53L1_CALIBRATION_OFFSET_ZERO_DISTANCE;
		param1 = 0;
		param2 = 0;
	break;
	default:
...
static int set_calibration_data_sysfs(char *name)
{
	...
	/* pass old version of calibration to let driver deal with */
	if ((rc == -1) &&((data.struct_version & 0xFFFF0000) == 0xECAB0000)) {
		printf("Read old calibration structure 0x%x\nContinue and pass it to the driver\n",data.struct_version);
		rc = 0;
	}
	if (rc == 0)
...
static int set_calibration_data_ioctl(char *name)
{
	...
	cal.is_read = 0;
	memset(&cal.data, 0, sizeof(cal.data));
	rc = read_file(name, (void *) &cal.data, sizeof(cal.data));
	/* pass old version of calibration to let driver deal with */
		if ((rc == -1) &&((cal.data.struct_version & 0xFFFF0000) == 0xECAB0000)) {
		printf("Read old calibration structure 0x%x\nContinue and pass it to the driver\n", cal.data.struct_version);
		rc = 0;
	}
	if (rc == 0)
		rc = stmvl53l1_calibration_data(dev_fd, &cal);
		...
		struct RangingDataProcess_t *pData)
{
	...
	rc = stmvl53l1_get_timing_budget(dev, &pData->TimingBudgetMicroSeconds);
	if (rc != 0) {
		printf("stmvl53l1_get_timing_budget() failed %d\n", rc);
	}
	return rc;
		...
		uint8_t *pConfLevel)
{
	...
	VL53L1_DistanceModes CurrentDM, NewDM;
	...
	if (pData->EnableDistanceModeSwitch) {
		stmvl53l1_get_distance_mode(dev, (uint32_t *) &CurrentDM);
		...
		if (CurrentDM != NewDM){
			stmvl53l1_stop(dev);
			stmvl53l1_set_distance_mode(dev, (uint32_t) NewDM);
			stmvl53l1_start(dev);
		}
	}
	...
	if (pData->EnableTimingBudgetSwitch) {
		...
		if ((SignalRate <= pData->SignalThreshold1) ||
			...
			stmvl53l1_set_timing_budget(dev,
