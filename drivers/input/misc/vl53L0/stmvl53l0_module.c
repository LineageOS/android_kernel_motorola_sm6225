...
#define VL53L0_IOCTL_OFFCALB		_IOWR('p', 0x03, unsigned int)
...
static void stmvl53l0_ps_read_measurement(struct stmvl53l0_data *data)
{
	...
	if (data->enableDebug)
		vl53l0_errmsg("range:%d, RtnRateMcps:%d, \
			err:0x%x,Dmax:%d,rtnambr:%d,time:%d,Spad:%d,SigmaLimit:%d\n",
			data->rangeData.RangeMilliMeter,
			data->rangeData.SignalRateRtnMegaCps,
			data->rangeData.RangeStatus,
			data->rangeData.RangeDMaxMilliMeter,
				...
				size_t count)
{
	...
	vl53l0_errmsg("Enter, enable_ps_sensor flag:%d\n",
		data->enable_ps_sensor);
	vl53l0_errmsg("enable ps senosr ( %ld)\n", val);
...
static DEVICE_ATTR(set_timing_budget, 0660/*S_IWUGO | S_IRUGO*/,
	stmvl53l0_show_timing_budget, stmvl53l0_store_set_timing_budget);
...
static DEVICE_ATTR(set_use_case, 0660/*S_IWUGO | S_IRUGO*/,
	stmvl53l0_show_use_case, stmvl53l0_store_set_use_case);
...
static DEVICE_ATTR(show_current_configuration, 0660/*S_IWUGO | S_IRUGO*/,
	stmvl53l0_show_current_configuration, NULL);
/* for work handler scheduler time */
			...
			void __user *p)
{
	...
	case VL53L0_IOCTL_REGISTER:
		...
		vl53l0_dbgmsg("VL53L0_IOCTL_REGISTER,	page number:%d\n", page_num);
		if (page_num != 0)
...
int stmvl53l0_setup(struct stmvl53l0_data *data)
{
...
#ifdef USE_INT
	/* init interrupt */
	gpio = data->client_object.gconf.cam_gpio_req_tbl[2].gpio;
	...
	if (irq < 0) {
		...
		rc = request_threaded_irq(irq, NULL,
				...
				(void *)data);
		if (rc) {
			vl53l0_errmsg("%d, Could not allocate STMVL53L0_INT ! result:%d\n",  __LINE__, rc);
			free_irq(irq, data);
			goto exit_free_irq;
		}
	}
	...
	vl53l0_errmsg("support ver. %s enabled\n", DRIVER_VERSION);
	vl53l0_dbgmsg("End");
